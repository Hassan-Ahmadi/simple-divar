...
async def update_ad(ad_id: int, ad_data: schemas.ADUpdate, db: Session = Depends(get_db)):
    db_ad = crud.get_ad_by_id(db=db, ad_id=ad_id)
    if db_ad:
        updated_ad = crud.update_ad(db=db, ad=db_ad, ad_data=ad_data.model_dump(exclude_unset=True))
        return updated_ad
    raise HTTPException(status_code=404, detail="Ad not found")
